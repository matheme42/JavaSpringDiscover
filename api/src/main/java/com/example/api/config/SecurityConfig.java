 ...
 * 
 * @Data Lombok annotation that automatically generates getters, setters,
 *       toString, equals, and hashCode methods for the class fields.
 * @Configuration Indicates that the class is a configuration class, providing
 *                bean definitions and other application configuration.
 * @EnableWebSecurity Enables Spring Security's web security features in the
 *                    application.
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
  ...
  SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    ...
    /**
     * Configures HTTP Basic authentication with default settings in the HTTP
    ...
    http.authorizeHttpRequests(req -> req
        .requestMatchers("/login/**", "/register/**", "/validate_accout/**", "/validate/**", "/forget_password/**",
            "/reset_password/**", "/refresh/**", "/error")
        .permitAll().requestMatchers("/**").hasAnyAuthority("USER", "ADMIN"));

    /**
     * Sets the UserDetailsService to be used for authentication in the HTTP
     * security configuration.
     */
     ...
     * - Sets a custom access denied handler and sets an HTTP status entry point for
     * unauthorized access.
     */
     ...
     * - Sets the session creation policy to STATELESS to indicate that no HTTP
    ...
    /**
     * Builds the HTTP security configuration and returns the configured
     * HttpSecurity object.
     */
    return http.build();
  }
